...
class LoginWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		self.loginFailureMsgDict={
			...
			"WRONGPIN"	: "LOGIN_FAILURE_WRONG_PIN",
			"WRPIN"		: "Wrong Pin, nur Zahlen akzeptiert",
			"WLPIN"		: "Die Laenge darf und muss mindestens 4 Zahlen haben.",
			"CRPIN"		: "Pin wurde erstellt",
		}
		...
		if not self.__LoadScript("locale/general/ui/LoginWindow.py"):
	...
	if app.__PIN_CODE_HAKYZZ__:
		def __DisconnectAndInputPin(self):
			if self.connectingDialog:
				self.connectingDialog.Close()
			self.connectingDialog = None
	...
	def __OpenServerBoard(self):
		...
		if app.__PIN_CODE_HAKYZZ__:
			if self.pinEditLine and self.pinEditLine.IsFocus():
	...
	def __OnClickLoginButton(self):
		...
		if len(pin)==0:
			self.PopupNotifyMessage("LOGIN_INPUT_PIN", self.SetPinEditLineFocus)
			return
